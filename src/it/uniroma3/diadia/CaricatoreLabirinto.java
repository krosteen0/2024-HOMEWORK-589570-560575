...
public class CaricatoreLabirinto {
	...
	/* prefisso della riga contenente il nome personaggio mago */
	private static final String PERSONAGGI_MAGO_MARKER = "Mago:";

	/* prefisso della riga contenente il nome personaggio cane */
	private static final String PERSONAGGI_CANE_MARKER = "Cane:";

	/* prefisso della riga contenente il nome personaggio strega */
			...
			try (Scanner scannerDiLinea = new Scanner(specifica)) {
				while (scannerDiLinea.hasNext()) {
					...
					check(scannerDiLinea.hasNext(),
							msgTerminazionePrecoce("la  stanza " + specifica + " non esiste\n"));
		...
		for (String specifica : separaStringheAlleVirgole(specificheStanze)) {
			...
			try (Scanner scannerDiLinea = new Scanner(specifica)) {
				while (scannerDiLinea.hasNext()) {
					...
					String nomeStanza = scannerDiLinea.next();
					check(scannerDiLinea.hasNext(),
							msgTerminazionePrecoce("la  direzione della stanza" + specifica + " non esiste\n"));
	...
	private void leggiECreaMaghi() throws FormatoFileNonValidoException {
		String specificheMago = this.leggiRigaCheCominciaPer(PERSONAGGI_MAGO_MARKER);
		if (specificheMago == null)
			return;
		for (String specifica : separaStringheAlleVirgole(specificheMago)) {
			...
			try (Scanner scannerDiLinea = new Scanner(specifica)) {
				while (scannerDiLinea.hasNext()) {
					...
					String nomeStanza = scannerDiLinea.next();
					check(scannerDiLinea.hasNext(),
							msgTerminazionePrecoce("problemini nella creazione del mago ...\n"));
					...
					AbstractPersonaggio personaggio = new Mago(nome, presentazione, new Attrezzo(attrezzo, 4));
	...
	private void leggiECreaCani() throws FormatoFileNonValidoException {
		String specificheCane = this.leggiRigaCheCominciaPer(PERSONAGGI_CANE_MARKER);
		if (specificheCane == null)
			return;
		for (String specifica : separaStringheAlleVirgole(specificheCane)) {
			...
			try (Scanner scannerDiLinea = new Scanner(specifica)) {
				while (scannerDiLinea.hasNext()) {
					...
					String nomeStanza = scannerDiLinea.next();
					check(scannerDiLinea.hasNext(),
							msgTerminazionePrecoce("problemini nella creazione del cane ...\n"));
					String nome = scannerDiLinea.next();
	...
	private void leggiECreaStreghe() throws FormatoFileNonValidoException {
		String specificheStrega = this.leggiRigaCheCominciaPer(PERSONAGGI_STREGA_MARKER);
		if (specificheStrega == null)
			return;
		for (String specifica : separaStringheAlleVirgole(specificheStrega)) {
